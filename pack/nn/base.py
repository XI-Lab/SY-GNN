...
from pack.nn.initializers import zeros_initializer
...
class GetItem(nn.Module):
    """
    Extracts a single item from the standard input dictionary.

    Args:
        key (str): Property to be extracted from input tensors.
    """
    ...
    def forward(self, input):
        """
        Args:
            input (dict of torch.Tensor): format dictionary of input tensors.
...
class Aggregate(nn.Module):
    ...
    def forward(self, input, mask=None):
        ...
        y = torch.sum(input, self.axis, keepdim=self.keepdim)
