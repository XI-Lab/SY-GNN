...
class BaseAtomsData(Dataset):
    ...
    def __init__(self, dbpath, subset=None, required_properties=[],
                 ...
                 load_charge=False, sym_tags=False):
        self.dbpath = dbpath
        self.subset = subset
    ...
    def create_splits(self, num_train=None, num_val=None, split_file=None):
        ...
        Returns:
            pack.data.AtomsData: training dataset
            pack.data.AtomsData: validation dataset
            pack.data.AtomsData: test dataset
        ...
        if split_file is not None and os.path.exists(split_file):
            ...
            assert num_train + num_val <= len(self), 'Dataset' + str(len(self)) + ' is smaller than num_train ' + str(
                num_train) + ' + num_val' + str(num_val) + '!'

            num_train = num_train if num_train > 1 else int(num_train * len(self))
            num_val = num_val if num_val > 1 else int(num_val * len(self))
    ...
    def __getitem__(self, idx):
        ...
        if self.sym_tags:
            properties['_tags'] = torch.LongTensor(at.get_tags())
        ...
        # TODO: consider modify the zpve and unit here
        return properties
    ...
    def get_properties(self, idx):
        ...
        if self.load_charge:
            if Structure.charge in row.data.keys():
                ...
                properties[Structure.charge] = torch.FloatTensor(np.array([0.], dtype=np.float32))
...
class AtomsLoader(DataLoader):
    ...
    def __init__(self, dataset, batch_size=1, shuffle=False, sampler=None,
        ...
        super(AtomsLoader, self).__init__(dataset, batch_size, shuffle, sampler,
    ...
    def get_statistics(self, property_names, per_atom=False, atomrefs=None):
        ...
        if type(atomrefs) is not list:
            # atomrefs = [atomrefs] * len(property_names)
            atomrefs_ = []
            for i in range(atomrefs.shape[1]):
                atomrefs_.append(atomrefs[:, i][:, None])
            atomrefs = atomrefs_
        with torch.no_grad():
...
class Structure:
    """
    Keys to access structure properties in `pack.data.AtomsData`
    """
    Z = '_atomic_numbers'
    charge = '_charge'
